...
namespace Acorn.Tests.DwarfTests;

public class DwarfRoundTripTests
{
    /// <summary>
    ///     最小编译单元往返测试：使用仅含基本字段的最小 DWARFFileData 编码后解码，验证所有字段一致
    /// </summary>
    [Fact]
    ...
    /// <summary>
    ///     多编译单元往返测试：包含多个编译单元的 DWARF 文件，编码后解码验证每个单元
    /// </summary>
    [Fact]
    ...
    /// <summary>
    ///     行号表往返测试：包含编译单元和行号表的完整 DWARF 文件，编码后解码验证
    /// </summary>
    [Fact]
    ...
    /// <summary>
    ///     多条目的编译单元往返测试：编译单元包含多个条目，每个条目有不同的属性，编码后解码验证
    /// </summary>
    [Fact]
