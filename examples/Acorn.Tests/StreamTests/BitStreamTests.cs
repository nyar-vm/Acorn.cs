...
namespace Acorn.Tests.StreamTests;
...
public class BitStreamRangedTests
{
    ...
    public void WriteRangedFloat_ThrowsOnOutOfRange()
    {
        var stream = new BitStream(8);
        var threw1 = false;
        try { stream.WriteRangedFloat(-0.1f, 0f, 1f, 8); }
        catch (ArgumentOutOfRangeException) { threw1 = true; }
        Assert.True(threw1);

        var stream2 = new BitStream(8);
        var threw2 = false;
        try { stream2.WriteRangedFloat(1.1f, 0f, 1f, 8); }
        catch (ArgumentOutOfRangeException) { threw2 = true; }
        Assert.True(threw2);
    }
}
...
public class BitStreamAdvancedTests
{
    ...
    {
        var stream = new BitStream(new byte[] { 0xFF });
        var threw = false;
        try { stream.ReadBits(9); }
    ...
    public void WriteBits_ThrowsOnInvalidBitCount()
    {
        var stream = new BitStream(4);
        var threw1 = false;
        try { stream.WriteBits(0, 0); }
        catch (ArgumentOutOfRangeException) { threw1 = true; }
        Assert.True(threw1);

        var stream2 = new BitStream(4);
        var threw2 = false;
        try { stream2.WriteBits(0, 33); }
        catch (ArgumentOutOfRangeException) { threw2 = true; }
        Assert.True(threw2);
    }
    ...
    public void ReadBits_ThrowsOnInvalidBitCount()
    {
        var stream = new BitStream(new byte[] { 0xFF });
        var threw1 = false;
        try { stream.ReadBits(0); }
        catch (ArgumentOutOfRangeException) { threw1 = true; }
        Assert.True(threw1);

        var stream2 = new BitStream(new byte[] { 0xFF, 0xFF, 0xFF, 0xFF, 0xFF });
        var threw2 = false;
        try { stream2.ReadBits(33); }
        catch (ArgumentOutOfRangeException) { threw2 = true; }
        Assert.True(threw2);
    }
