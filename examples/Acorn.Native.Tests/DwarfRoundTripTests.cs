...
public sealed class DwarfRoundTripTests
{
    ...
    {
        var data = CreateMinimalCompilationUnit();
    ...
    {
        var data = CreateCompilationUnitWithAttributes();
        ...
        Assert.True(language.Value is byte langVal && langVal == 2);

        var stmtList = entry.Attributes.FirstOrDefault(a => a.Name == 0x10u);
        Assert.NotNull(stmtList);
        Assert.True(stmtList.Value is uint stmtVal && stmtVal == 0);
    }

    [Fact]
    public void EncodeDecode_MultipleEntries_Roundtrip()
    {
        var data = CreateCompilationUnitWithMultipleEntries();
    ...
    {
        var data = CreateLineNumberTable();
    ...
    public void EncodeDecode_CUAndLineTable_Roundtrip()
    {
        var data = new DWARFFileData
        {
            CompilationUnits = [CreateMinimalCU()],
            LineNumberTables =
            [
                new DWARFLineNumberTableData
                {
...
#pragma warning restore xUnit1026
    {
        ...
        else if (value is int i)
        {
            switch (attr.Value)
            {
                ...
                default:
                    Assert.Fail($"属性值类型不匹配: 期望 int({i}), 实际 {attr.Value?.GetType().Name}({attr.Value})");
                    break;
            }
        }
    ...
    private static DWARFCompilationUnitData CreateMinimalCU()
    {
    ...
    private static DWARFFileData CreateMinimalCompilationUnit()
    {
    ...
    private static DWARFFileData CreateCompilationUnitWithAttributes()
    {
        return new DWARFFileData
        {
            CompilationUnits =
    ...
    private static DWARFFileData CreateCompilationUnitWithMultipleEntries()
    {
        return new DWARFFileData
        {
            CompilationUnits =
    ...
    private static DWARFFileData CreateLineNumberTable()
    {
        return new DWARFFileData
        {
            CompilationUnits = [],
