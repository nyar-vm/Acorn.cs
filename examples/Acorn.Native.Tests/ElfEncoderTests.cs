...
public sealed class ElfEncoderTests
{
    ...
    {
        var data = CreateMinimalElfData(is64: false);
        var encoder = new ElfEncoder();
        var bytes = encoder.Encode(data);
    ...
    public void Encode_WithSectionContent_IncludesContentBytes()
    {
        ...
        var data = CreateElfWithSectionContent(textContent);
    ...
    public void Encode_MultipleSections_AllContentPresent()
    {
        ...
        var data = CreateElfWithMultipleSections(textContent, dataContent);
        var encoder = new ElfEncoder();
        var bytes = encoder.Encode(data);
    ...
    {
        var data = CreateMinimalElfData(is64: true, machine: 62);
    ...
    {
        var data = CreateMinimalElfData(is64: true, entryPoint: 0x400100);
    ...
    public void Encode_WithNamedSections_StringTableContainsNames()
    {
        ...
        Assert.True(shstrtabFound, "字符串表中未找到 .shstrtab 名称");
    }
    ...
    {
        var data = CreateMinimalElfData(is64, machine: machine);
    ...
    private static ELFHeaderData CreateMinimalElfHeader(bool is64, ushort machine = 0, ulong entryPoint = 0)
    {
    ...
    private static ELFFileData CreateMinimalElfData(bool is64, ushort machine = 0, ulong entryPoint = 0)
    {
        return new ELFFileData
        {
            Header = CreateMinimalElfHeader(is64, machine, entryPoint),
            ProgramHeaders = [],
            SectionHeaders = []
        };
    }

    private static ELFFileData CreateElfWithSectionContent(byte[] textContent)
    {
        var headerSize = 64;
        var phSize = 56;
    ...
    private static ELFFileData CreateElfWithMultipleSections(byte[] textContent, byte[] dataContent)
    {
        var headerSize = 64;
        var phSize = 56;
        var textOffset = headerSize + phSize;
