...
public sealed class ElfEncoderAdvancedTests
{
    ...
    {
        var data = CreateMinimalElfData(expect64, machine: machine);
    ...
    {
        var data = CreateMinimalElfData(is64: true, type: 3);
        var encoder = new ElfEncoder();
        var bytes = encoder.Encode(data);
    ...
    public void Encode_SectionAlignment_PreservesValue()
    {
        ...
        var data = CreateElfWithTextSection(textContent, alignment: 32);
    ...
    public void Encode_LargeSectionContent_ProducesCorrectOutput()
    {
        ...
        var data = CreateElfWithTextSection(textContent);
        var encoder = new ElfEncoder();
        var bytes = encoder.Encode(data);
    ...
    public void Encode_ProgramHeaderType_PreservesValue(uint phType)
    {
        ...
        {
            Header = CreateMinimalElfHeader(true, programHeaderCount: 1),
            ProgramHeaders =
            [
    ...
    public void Encode_StringTable_ContainsAllSectionNames()
    {
        ...
        var data = CreateElfWithTextAndDataSections(textContent, dataContent);
        var encoder = new ElfEncoder();
        var bytes = encoder.Encode(data);
        ...
        Assert.True(ContainsBytes(bytes, searchText), ".text 未在字符串表中找到");
        Assert.True(ContainsBytes(bytes, searchData), ".data 未在字符串表中找到");
        Assert.True(ContainsBytes(bytes, searchShstrtab), ".shstrtab 未在字符串表中找到");
    }
    ...
    {
        var header = CreateMinimalElfHeader(true);
        var data = new ELFFileData
        {
    ...
    {
        var textContent = new byte[] { 0xC3 };
        var data = CreateElfWithTextSection(textContent, flags: 6);
    ...
    public void Encode_DataSectionFlags_IsWriteAndAlloc()
    {
        ...
        var data = CreateElfWithTextAndDataSections([0xC3], dataContent);
    ...
    {
        var data = CreateMinimalElfData(is64: true, entryPoint: entryPoint);
        var encoder = new ElfEncoder();
        var bytes = encoder.Encode(data);
    ...
    public void Encode_NullSectionHeader_ProducesValidOutput()
    {
        var data = new ELFFileData
        {
            Header = CreateMinimalElfHeader(true, sectionHeaderCount: 1),
            ProgramHeaders = [],
            SectionHeaders =
            [
    ...
    private static ELFFileData CreateMinimalElfData(bool is64, ushort machine = 0, ulong entryPoint = 0, ushort type = 2)
    {
    ...
    private static ELFFileData CreateElfWithTextSection(byte[] textContent, ulong flags = 6, ulong alignment = 16)
    {
        var headerSize = 64;
        var phSize = 56;
    ...
    private static ELFFileData CreateElfWithTextAndDataSections(byte[] textContent, byte[] dataContent)
    {
        var headerSize = 64;
        var phSize = 56;
        var textOffset = headerSize + phSize;
