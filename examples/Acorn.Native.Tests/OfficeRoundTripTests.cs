...
public sealed class OfficeRoundTripTests
{
    ...
    public void EncodeDecode_ChineseName_Roundtrip()
    {
        var original = new ExcelWorkbookData
        {
            Sheets = new List<ExcelSheetData>
            {
                new() { Name = "员工表", RowCount = 0, ColumnCount = 0, Rows = [] },
                new() { Name = "工资表", RowCount = 0, ColumnCount = 0, Rows = [] }
            }
        };
        ...
        Assert.Equal("员工表", result.Sheets[0].Name);
        Assert.Equal("工资表", result.Sheets[1].Name);
    }
