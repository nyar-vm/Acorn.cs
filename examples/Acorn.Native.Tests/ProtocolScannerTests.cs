using Acorn.Frame;
using Acorn.MySql.Scanner;
using Acorn.PostgreSql.Scanner;
using Acorn.Redis.Scanner;
...
public sealed class MySqlScannerTests
{
    ...
    {
        var packet1 = BuildMySqlPacket([0x0A, 0x0B], 0);
        var packet2 = BuildMySqlPacket([0x0C, 0x0D, 0x0E], 1);
        var packet3 = BuildMySqlPacket([0x0F], 2);
    ...
    {
        var packet1 = BuildMySqlPacket([0x01, 0x02], 0);
        var packet2 = BuildMySqlPacket([0x03, 0x04, 0x05, 0x06], 1);
    ...
    public void Scan_LargePayload_ReadsCorrectly()
    {
        ...
        var packet = BuildMySqlPacket(payload, 0);
    ...
    private static byte[] BuildMySqlPacket(byte[] payload, byte sequenceId)
    {
...
public sealed class PostgreSqlScannerTests
{
    ...
    {
        var msg1 = BuildPostgreSqlMessage((byte)'R', [0x00, 0x00, 0x00, 0x00]);
        var msg2 = BuildPostgreSqlMessage((byte)'K', []);
        var msg3 = BuildPostgreSqlMessage((byte)'Z', [0x05]);
    ...
    public void Scan_Statistics_AccumulatesCorrectly()
    {
        var msg1 = BuildPostgreSqlMessage((byte)'R', [0x00, 0x00, 0x00, 0x08]);
        var msg2 = BuildPostgreSqlMessage((byte)'Z', [0x05]);
    ...
    public void Scan_PartialHeader_ReturnsNoFrame()
    {
        var scanner = new PostgreSqlScanner([0x52, 0x00, 0x00]);
    ...
    {
        var message = BuildPostgreSqlMessage((byte)'R', [0x00, 0x00, 0x00, 0x00]);
    ...
    private static byte[] BuildPostgreSqlMessage(byte msgType, byte[] body)
    {
