...
public class XlsEncoderDecoderRoundTripTests
{
    ...
    public void RoundTrip_MultipleSheetWorkbook()
    {
        var original = new ExcelWorkbookData
        {
            Sheets =
            [
                new ExcelSheetData { Name = "数据表", RowCount = 0, ColumnCount = 0, Rows = [] },
                new ExcelSheetData { Name = "配置表", RowCount = 0, ColumnCount = 0, Rows = [] },
        ...
        Assert.Equal("数据表", decoded.Sheets[0].Name);
        Assert.Equal("配置表", decoded.Sheets[1].Name);
    ...
    public void RoundTrip_UnicodeSheetNames()
    {
        var original = new ExcelWorkbookData
        {
            Sheets =
            [
                new ExcelSheetData { Name = "角色属性", RowCount = 0, ColumnCount = 0, Rows = [] },
                new ExcelSheetData { Name = "アイテム", RowCount = 0, ColumnCount = 0, Rows = [] }
            ]
        };
        ...
        Assert.Equal("角色属性", decoded.Sheets[0].Name);
        Assert.Equal("アイテム", decoded.Sheets[1].Name);
    }
}
...
{
    private static byte[] BuildMinimalXls(params string[] sheetNames)
    {
        var data = new ExcelWorkbookData
        {
    ...
    {
        var bytes = BuildMinimalXls("Sheet1");
