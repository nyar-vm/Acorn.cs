...
本文档描述 Acorn Binary Framework 的核心接口与基础设施。这些接口和类是框架的**稳定基础设施**，用户和生成器共同依赖，**不会随业务变化而修改**。
...
`Acorn.Core` 的接口设计已经过多个项目的验证，并承载了多种复杂协议（MySQL、PostgreSQL、ZeroMQ、GLTF、SPIR-V、WASM）的手写和生成式解析。其稳定性体现在：

- **基础设施**（`ByteBuffer`、`ICodec<T>`、`IFrameProtocol`、管道适配器）自 1.0 起未发生断裂式变更
- **声明式特性系统** 的生成契约稳定，扩展不破坏旧代码
- **专用协议** 永远以独立包形式发布，不会污染核心抽象

Acorn Binary Framework **无需修改内核即可进化**。未来新增 `Acorn.FlatBuffers`、`Acorn.Bencode` 等包只需遵循现有接口，而无需核心库为你妥协。
