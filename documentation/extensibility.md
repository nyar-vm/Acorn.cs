...
本文档描述 Acorn Binary Framework 的扩展机制。框架核心绝不尝试直接提供所有协议的完整解析器，但提供了构建它们的 **全部基础设施**。
...
### 包结构示例：Acorn.Protobuf

```
Acorn.Protobuf/
├── Acorn.Protobuf.csproj
├── Codec/
│   └── ProtobufVarintCodec.cs
├── Protocol/
│   └── ProtobufFrameProtocol.cs
├── Attributes/
│   ├── ProtoMessageAttribute.cs
...
public sealed class ProtoFieldAttribute : Attribute
{
    ...
    public ProtoWireType WireType { get; set; } = ProtoWireType.Varint;

    public ProtoFieldAttribute(int index)
    {
        Index = index;
    }
}
...
public partial struct Person
{
    ...
    [ProtoField(3, WireType = ProtoWireType.LengthDelimited)]
...
| 包 | 协议 | 说明 |
|---|---|---|
| `Acorn.Protobuf` | Protocol Buffers | Google 的二进制序列化格式 |
...
| `Acorn.Wasm` | WebAssembly | WebAssembly 二进制格式 |
| `Acorn.Gltf` | glTF | glTF 2.0 二进制格式 |
| `Acorn.Spine` | Spine | Spine 骨骼动画格式 |
| `Acorn.Live2D` | Live2D | Live2D Cubism 模型格式 |
