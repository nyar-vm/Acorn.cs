...
namespace Acorn.DWARF.Scanner;

/// <summary>
///     DWARF 文件扫描器，提供对调试信息的快速结构扫描。
/// </summary>
public class DWARFScanner
{
    ...
    public static string Scan(byte[] data)
    {
        ...
        result.AppendLine("✅ 有效的 DWARF 数据");
        result.AppendLine();
        ...
        try
        {
            ...
            result.AppendLine($"📋 编译单元数量: {dwarfFile.CompilationUnits.Count}");

            if (dwarfFile.CompilationUnits.Count > 0)
            {
                result.AppendLine();
                ...
                foreach (var unit in dwarfFile.CompilationUnits)
                {
                    ...
                    result.AppendLine($"     单元长度: {unit.UnitLength} bytes");
                    result.AppendLine($"     地址大小: {unit.AddressSize} bytes");
                    ...
                    if (unit.Entries.Count > 0)
                    {
                        ...
                        {
                            result.AppendLine($"       - {GetTagName(entry.Tag)}");
                        }
                        if (unit.Entries.Count > 5)
                        {
                            result.AppendLine($"       ... 等 {unit.Entries.Count - 5} 个条目");
                        }
                    }
                }
            }
            ...
            result.AppendLine($"📊 行号表数量: {dwarfFile.LineNumberTables.Count}");
        }
        catch (Exception ex)
        {
